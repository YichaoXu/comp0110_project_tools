...
class CoChangedCommitMeasurement(CoChangedDataMeasurement):
    ...
    __SELECT_TESTED_CHANGED_COMMITS = '''
    ...
    def __init__(self, path_to_db: str, path_to_csv: str):
        self.__tested_change_commits: Dict[int, List[int]] = dict()
        self.__co_changed_commits: Dict[Tuple[int, int], List[int]] = dict()
    ...
    def _measure(self) -> None:
        ...
        for tested_id, test_id in self._ground_truth_links.keys():
            select_sql = self.__SELECT_TESTED_CHANGED_COMMITS
            exe_result = db_cursor.execute(select_sql, {'tested_id': tested_id})
            ...
            self.__tested_change_commits[tested_id] = possible_commit_ids
            ...
            possible_commit_hashes = [row[0] for row in exe_result if exe_result is not None and len(row) >= 1]
            possible_commit_ids = self.__from_hashes_to_ids(possible_commit_hashes)
            self.__co_changed_commits[(tested_id, test_id)] = possible_commit_ids
        return None
    ...
    @property
    def tested_change_commits(self) -> Dict[int, List[int]]:
        return self.__tested_change_commits

    @property
    def co_change_commits(self) -> Dict[Tuple[int, int], List[int]]:
