...
class PrecisionRecallMeasurement(StrategyWithGroundTruthMeasurement):
    ...
    def precision(self) -> float:
        if self.__all_predict_link_num == 0: return 0
        return self.__valid_predict_link_num / self.__all_predict_link_num

    @property
    def recall(self) -> float:
        if self.__ground_truth_link_num == 0: return 0
    ...
    def fn_score(self, n) -> float:
        if self.precision == 0 and self.recall == 0: return 0
...
class PrecisionRecallMeasurementClassLevel(StrategyWithGroundTruthMeasurementClassLevel):
