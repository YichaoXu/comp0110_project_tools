...
class MeanAbsoluteAndSquaredErrorMeasurement(StrategyWithGroundTruthMeasurement):
    ...
    def __get_predicate_score(self, test: GroundTruthMethodName, tested: GroundTruthMethodName) -> float:
        cursor = self._predict_database.cursor()
        select_sql = self.__SELECT_ALL_CANDIDATE_WITH_SCORE_SQL.format(strategy_name=self.__strategy_name)
        possible_link = cursor.execute(select_sql, {
