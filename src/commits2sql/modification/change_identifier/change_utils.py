...
class CodeAnalyser(object):
    ...
    def __binary_search(self, char_num: int, start_line: int, end_line: int):
        ...
        if self.__line_num_char_num_dict[mid] <= char_num < self.__line_num_char_num_dict[mid + 1]: return mid
        if self.__line_num_char_num_dict[mid] > char_num: return self.__binary_search(char_num, start_line, mid)
        if self.__line_num_char_num_dict[mid + 1] < char_num: return self.__binary_search(char_num, mid + 1, end_line)

    def get_char_num_range(self, line_num: int) -> Optional[Tuple[int, int]]:
        if len(self.__line_num_char_num_dict) <= line_num + 1: return -1, -1
    ...
    def get_line_num_of_class(self, name: str) -> int:
        for line_num, class_name in self.line_num_classname_dict.items():
            if name == class_name: return line_num
        return -1

    def get_classname_at_line(self, num: int) -> Optional[str]:
        return self.line_num_classname_dict[num] if num in self.line_num_classname_dict else None
...
class ChangeClassifier(object):
    ...
    __REGEX_MATCH_NAME = r'^SimpleName: (?P<old_name>\w+) \[(?P<old_start>\d+),(?P<old_end>\d+)\]\s*' \
                         r'SimpleName: (?P<new_name>\w+) \[(?P<new_start>\d+),(?P<new_end>\d+)\]'
    __REGEX_UPDATE_NAME = r'^SimpleName:.*\[(?P<old_start>\d+),(?P<old_end>\d+)\]\s*' \
