...
class CommitsDataMeasurement(AbstractMeasurement):
    ...
    def __init__(self, path_to_db: str):
        ...
        self.__change_type_z_table: Dict[str, int] = {
            "ADD": 1,
            "MODIFY": 2,
            "RENAME": 3,
        }
    ...
    def _measure(self) -> None:
        exec_res = self._predict_database.cursor().execute(
            self.__SELECT_ALL_CHANGED_COMMITS_OF_METHOD_SQL_STMT
        )
        for method_id, file_path, change_type, commit_id in exec_res:
            ...
            coordinates = (
                self.__coordinates_for_test
                if self.__is_test(file_path)
                else self.__coordinates_for_tested
            )
    ...
    def __from_method_package_signature_to_x(self, package_name: str) -> int:
        if package_name not in self.__method_package_x_table:
            self.__method_package_x_table[package_name] = (
                len(self.__method_package_x_table) + 1
            )
