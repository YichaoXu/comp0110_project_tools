...
def mining_jfreechart():
    ...
    specific_paths = {
        "tested_path": "source/org/jfree/chart/%",
        "test_path": "tests/org/jfree/chart/%",
    }
    sql2linker.run(
        LinkStrategy.COCREATE, LinkBase.FOR_COMMITS, parameters=specific_paths
    )
    sql2linker.run(
        LinkStrategy.COCHANGE, LinkBase.FOR_COMMITS, parameters=specific_paths
    )
...
def mining_ant():
    path_to_comp0110 = os.path.expanduser("~/Project/PycharmProjects/comp0110")
    DataMiner(
        f"{path_to_comp0110}/.tmp", f"{path_to_comp0110}/example_repositories/ant"
...
if __name__ == "__main__":
    path_to_comp0110 = os.path.expanduser("~/Project/PycharmProjects/comp0110")
    DataMiner(
        f"{path_to_comp0110}/.tmp", f"{path_to_comp0110}/example_repositories/guava"
    ).mining()
