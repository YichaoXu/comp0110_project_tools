...
def main():
    ...
    with zipfile.ZipFile(f'{input_dir}/commons_lang.zip', 'r') as target_zip:
        ...
        TraceabilityPredictor(database_path).run(LinkStrategy.COCHANGE, LinkBase.FOR_COMMITS)
        ground_truth_csv_path = f'{input_dir}/gt_commons_lang.csv'
        # 3. Evaluating
        evaluator = LinkEvaluator(database_path, ground_truth_csv_path)
        print(evaluator.precision_recall_and_f1_score_of_strategy('links_commits_based_cochanged'))
